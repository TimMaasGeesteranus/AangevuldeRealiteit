...
Het component *Applicatie* betreft een Unity applicatie voor Android met de volgende functionaliteiten:
...
De location-based-Ar plugin waar gebruik van is gemaakt om de location-based implementatie te realiseren. Waarbij de keuze is gemaakt om het als assembly aan het project toe te voegen, wegens het feit dat er anders circular dependencies zouden onstaan met de base-assembly *CsharpAssembly*.
...
###### WatsonUnitySDK
De bestanden van de Watson services. Waarbij de applicatie gebruikmaakt van de [TTS (text-to-speech) API](https://cloud.ibm.com/apidocs/text-to-speech/text-to-speech).
...
Zo staan alle monobehavior scripts in de presentation directory en wordt vanuit daar de Business layer benaderd.

Daarnaast wordt er ook rekening gehouden met de scene's in het project. De presentation layer bestaat uit de verschillende scenes in de applicatie, waardoor alles goed georganiseerd bijlft ondanks uitbereidingen.
