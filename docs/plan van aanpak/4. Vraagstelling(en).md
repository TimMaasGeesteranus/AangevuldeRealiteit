# Deelvraag 1: Hoe kan door middel van de Google maps API location based AR worden gerealiseerd?
## Motivatie
Om location based AR te kunnen verwezenlijken moet onderzocht worden op welke manier dit kan. Er is voor de API van Google Maps gekozen omdat deze met afstand de meeste gegevens bevat. Ook is er veel te vinden over het gebruik van de API. 

## Relevantie
De applicatie zal gaan werken met location based AR. Hiervoor is data nodig. Deze zal via de API worden verkregen. Door middel van deze data kan met AR de juiste gegevens op de juiste locaties weergegeven worden. 
...
In de doelstelling staat beschreven dat er een aantal onderzoeken zullen plaatsvinden om uit te zoeken wat de beste manier is om API's te gebruiken. 
