...
public class CameraSettingsController : SampleController
{
    ...
    private bool _cameraListSupported = false;

    public void OnSDKInitialized() {
        _cameraListSupported = Camera.IsCameraListSupported();

        if (_cameraListSupported) {
            Camera.GetCameraList((cameraCount) => {
                PositionDropdown.ClearOptions();
                List<string> dropOptions = new List<string>();
                for (int i = 0; i < cameraCount; i++) {
                    dropOptions.Add(Camera.GetCameraName(i));
                }
                PositionDropdown.AddOptions(dropOptions);
            });
        }
    }
    ...
    public void OnPositionChanged(int newPosition) {
        if (_cameraListSupported) {
            Camera.SetActiveCamera(newPosition);
        } else {
