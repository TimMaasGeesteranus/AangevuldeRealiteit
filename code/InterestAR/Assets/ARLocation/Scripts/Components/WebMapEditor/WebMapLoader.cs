...
namespace ARLocation {
	...
	void LoadXmlFile()
	{
		...
		Debug.LogError("[ARLocation#WebMapLoader]: Failed to parse XML file.");
	    }
		...
		double lat = double.Parse(node["lat"].InnerText);
		double lng = double.Parse(node["lng"].InnerText);
		double altitude = double.Parse(node["altitude"].InnerText);
		string altitudeMode = node["altitudeMode"].InnerText;
		string name = node["name"].InnerText;
		string meshId = node["meshId"].InnerText;
		float movementSmoothing = float.Parse(node["movementSmoothing"].InnerText);
